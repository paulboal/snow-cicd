...
SELECT 'dynamic_table_has_rows' AS TEST_NAME,
       CASE WHEN COUNT(*) > 0 THEN 'PASS' ELSE 'FAIL' END AS RESULT
FROM CICD_DEMO.__SCHEMA__.PROPERTY_STATUS_BY_ZIP;
...
FROM (
    ...
    FROM CICD_DEMO.__SCHEMA__.PROPERTY p
    LEFT JOIN CICD_DEMO.__SCHEMA__.PROPERTY_EVENT pe ON pe.PROPERTY_ID = p.PROPERTY_ID
...
FROM (
    ...
    FROM (
        SELECT DISTINCT ZIP_CODE FROM CICD_DEMO.__SCHEMA__.PROPERTY
        MINUS
        SELECT ZIP_CODE FROM CICD_DEMO.__SCHEMA__.PROPERTY_STATUS_BY_ZIP
    )
);
...
FROM (
    SELECT
        (SELECT SUM(TOTAL_PROPERTIES) FROM CICD_DEMO.__SCHEMA__.PROPERTY_STATUS_BY_ZIP) AS dt_total,
        (SELECT COUNT(*) FROM CICD_DEMO.__SCHEMA__.PROPERTY) AS prop_count
);
